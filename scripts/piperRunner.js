...
const SCRIPT_PATH = path.resolve(__dirname, 'piper_tts_engine.py');
...
export function synthesizePiper(text, voice = 'ngochuyen', rate = '+0%') {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      if (code !== 0) {
        return reject(new Error(`Piper python process exited with code ${code}: ${stderrData}`));
      }
      try {
        const result = JSON.parse(stdoutData);
        if (result.error) {
          return reject(new Error(`Piper error: ${result.error}`));
        }
        resolve(result);
      } catch (e) {
        reject(new Error(`Failed to parse Piper output: ${e.message}\nRaw: ${stdoutData.slice(0, 200)}`));
      }
    });
